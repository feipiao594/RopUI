...
static void armTimerKqueue(const std::shared_ptr<KqueueWorkerTimerState>& state,
...
static void disarmTimerKqueue(const std::shared_ptr<KqueueWorkerTimerState>& state) {
    if (!state || state->kq_fd < 0) return;
    struct kevent kev;
...
static uint64_t drainTimerEvents(const std::shared_ptr<KqueueWorkerTimerState>& state) {
...
KqueueWorkerTimerWatcher::KqueueWorkerTimerWatcher(IOWorker& worker, std::function<void()> callback)
    : IWorkerWatcher(worker) {
    state_ = std::make_shared<KqueueWorkerTimerState>();
    state_->kq_fd = ::kqueue();
    if (state_->kq_fd < 0) {
...
KqueueWorkerTimerWatcher::~KqueueWorkerTimerWatcher() {
    stop();
    clearSpec();
    source_.reset();
    state_.reset();
...
void KqueueWorkerTimerWatcher::start() {
    if (attached_) return;
    attachSource(source_);
    attached_ = true;
}

void KqueueWorkerTimerWatcher::stop() {
    if (!attached_) return;
    detachSource(source_);
    attached_ = false;
}

void KqueueWorkerTimerWatcher::setSpec(std::chrono::nanoseconds initial_delay,
...
void KqueueWorkerTimerWatcher::clearSpec() {
    if (!state_) return;
    state_->promote_to_periodic = false;
    state_->periodic_ms = 0;
...
void KqueueWorkerTimerWatcher::createSource() {
    auto state = state_;
